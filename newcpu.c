...
static uae_s32 ShowEA (void *f, uae_u16 opcode, int reg, amodes mode, wordsizes size, char *buf)
{
	...
	break;
     case absw:
	sprintf (buffer, "$%08x", (unsigned int)(uae_s32)(uae_s16)get_iword_1 (&regs, m68kpc_offset));
	m68kpc_offset += 2;
	break;
     case absl:
	sprintf (buffer, "$%08x", (unsigned int)get_ilong_1 (&regs, m68kpc_offset));
	m68kpc_offset += 4;
	break;
     case imm:
	switch (size){
...
//ric perf
#define get_iword0(regs) do_get_mem_word((uae_u16 *)((regs)->pc_p))
//end ric
...
static void m68k_run_2 (void)
{
	...
	PROFILINGBEGIN
#endif
	uae_u32 opcode = get_iword0(r);
//ric perf	uae_u32 opcode = get_iword (r, 0);
//ric perf	count_instr (opcode);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+2,"m68k_run2_count")
#endif
	cycles = (*cpufunctbl[opcode])(opcode, r);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+3,"m68k_run2_cycles")
#endif
	cycles &= cycles_mask;
    cycles |= cycles_val;
	do_cycles (cycles);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+4,"m68k_run2_do_cycles")
#endif
	if (r->spcflags)
	{
	    if (do_specialties (cycles, r))
...
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+5,"m68k_run2_special")
#endif
    }
}

static void m68k_run_2_perf (void)
{
    struct regstruct *r = &regs;
	int cycles;
...
//ric perf	count_instr (opcode);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+12,"m68k_run2perf_count")
#endif
	cycles = (*cpufunctbl[opcode])(opcode, r);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+13,"m68k_run2perf_cycles")
#endif
//ric perf	cycles &= cycles_mask;
//ric perf    cycles |= cycles_val;
	do_cycles (cycles);
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+14,"m68k_run2perf_do_cycles")
#endif
	if (r->spcflags)
	{
	    if (do_specialties (cycles, r))
...
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+15,"m68k_run2perf_special")
#endif
    }
}
...
// thinkp
	...
	run_func = (currprefs.cpu_level == 0 && currprefs.cpu_cycle_exact	? m68k_run_1_ce :
		    ...
		    currprefs.cpu_compatible					? m68k_run_2p :
										  m68k_run_2);
#else
//ric perf	run_func = (currprefs.cpu_cycle_exact && currprefs.cpu_level == 0	? m68k_run_1_ce :
//ric perf		    currprefs.cpu_compatible > 0 && currprefs.cpu_level == 0	? m68k_run_1 :
//ric perf		    currprefs.cpu_level >= 2 && currprefs.cachesize		? m68k_run_2a :
//ric perf		    currprefs.cpu_compatible					? m68k_run_2p :
//ric perf										  m68k_run_2);
	run_func = (currprefs.cpu_cycle_exact && currprefs.cpu_level == 0	? m68k_run_1_ce :
		    ...
		    currprefs.cpu_compatible					? m68k_run_2p :
		    currprefs.m68k_speed >= 0					? m68k_run_2 :
										  m68k_run_2_perf);

#endif
	run_func ();
#ifdef PROFILING
	PROFILINGENDNAME(PROFILINGINDEX+6,"m68k_go")
#endif
    }
    in_m68k_go--;
}
