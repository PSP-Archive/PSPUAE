...
extern int g_kickstart_rom;
extern int changeKickstart(int version);
...
void default_prefs (struct uae_prefs *p, int type)
{
...
#endif
    p->gfx_framerate = 1;
    p->gfx_width_fs = 360; //ric
    p->gfx_height_fs = 272; //ric
    p->gfx_width_win = 360; //ric
    p->gfx_height_win = 272; //ric
    p->gfx_lores = 1; //ric
    p->gfx_linedbl = 0; //ric
    p->gfx_afullscreen = 1;
    p->gfx_pfullscreen = 0;
    ...
    //strcpy (p->romfile, "KICKS/Kick13.rom"); //ric //FOL - Added KICKS/
#ifdef PSP
    changeKickstart(g_kickstart_rom);
    strcpy(p->romfile, changed_prefs.romfile);
#endif
    strcpy (p->keyfile, "");
    strcpy (p->romextfile, "");
    strcpy (p->flashfile, "");
